...
export function rankAllNews(items) {
  return [...new Map(items.map(n => [n.issue,n])).values()].map(n => ({...n, score: Math.round(n.scores[0]*.35+n.scores[1]*.30+n.scores[2]*.25+n.scores[3]*.10), duplicates: items.filter(x=>x.issue===n.issue).length-1 })).sort((a,b)=>b.score-a.score);
}
