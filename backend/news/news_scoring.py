...
STRING={'type':'string'}
SCORE_SCHEMA=array_schema('scores',{'article_id':STRING,'summary':STRING,**{k:{'type':'integer'} for k in ('technical_score','organization_score','impact_score')}})
    ...
    async with AsyncExitStack() as stack:
        ...
        for article in articles:
            ...
            if cached and evaluation_complete(cached):
                article.update({k:cached[k] for k in ('article_id','title','published_at','content','summary','image_url')})
                scores.append({k:cached[k] for k in ('article_id','summary','request_id',*BASE_SCORES)}|weighted_scores(cached))
        ...
        for score in fresh:
            ...
            article.update({k:stored[k] for k in ('article_id','summary')})
            scores.append({k:stored[k] for k in ('article_id','summary','request_id',*BASE_SCORES)}|weighted_scores(stored))
    mapping={a['article_id']:a for a in articles}
