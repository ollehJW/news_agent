...
def compact_articles(db):
    ...
    if 'favicon_url' not in columns and 'image_storage_path' not in columns and columns.index('summary')==columns.index('total_score')+1:return
    ...
    db.execute(ddl)
    fields=[r['name'] for r in db.execute('PRAGMA table_info(articles_compact)')]
    names=','.join(fields)
